...
| --- | --- | --- | --- | --- | --- |
| D1 | Windows Codex | WezTerm | PowerShell | Codex CLI | Windows PowerShell adapters such as `$env:POLYDEV_SCRIPTS\start-codex-investigation.ps1` and `$env:POLYDEV_SCRIPTS\start-codex-worktree.ps1` |
| D2 | Windows ClaudeCode | WezTerm | Git Bash | Claude Code | Bash scripts such as `$POLYDEV_SCRIPTS/spawn-session.sh` and `$POLYDEV_SCRIPTS/spawn-agent.sh` |
| D3 | Linux Codex | tmux | bash | Codex CLI | Bash scripts such as `$POLYDEV_SCRIPTS/spawn-codex.sh` |
| D4 | Linux ClaudeCode | tmux | bash | Claude Code | Bash scripts such as `$POLYDEV_SCRIPTS/spawn-session.sh` and `$POLYDEV_SCRIPTS/spawn-agent.sh` |
...
### Script Path
Always call scripts through the installed package script directory for the current runtime. Keep the variable local to the command being run; do not write it into user shell profiles or user-level environment variables.
...
"$POLYDEV_SCRIPTS/spawn-agent.sh" research --prompt "Inspect repository only." --report .agent-reports/research.md --cwd .
"$POLYDEV_SCRIPTS/spawn-session.sh" my-project feature-auth .worktrees/feature-auth docs/plans/auth.md
```

Codex on Linux (D3, bash):
```bash
POLYDEV_SCRIPTS="$HOME/.codex/polydev/scripts"
"$POLYDEV_SCRIPTS/spawn-codex.sh" research --prompt "Inspect repository only." --cwd . --output .agent-reports/codex.md
```
