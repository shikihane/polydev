...
#
# Usage:
#   spawn-session.sh <workspace> <branch_name> <worktree_path> <plan_file> [verify_level] [verify_fallback] [verify_commands] [--verbose]
#
...
backup_task_toon() {
  local task_file="$1"
  local backup_dir
  backup_dir="$(dirname "$task_file")/.task_backups"
  ...
  (
    cd "$backup_dir"
    ls -t task.toon.*.bak 2>/dev/null | tail -n +11 | while read -r f; do
      rm -f "$f"
    done
  ) 2>/dev/null || true
}
...
# Fixed sleep is unreliable; Claude can take several seconds to boot.
