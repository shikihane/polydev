...
This reference is for the agent running inside a Polydev worktree. It is provider-neutral: Codex CLI, Cursor, OpenCode, Claude Code, Gemini CLI, or another adapter may be the executing agent.

Announce: "I'm executing the plan in this worktree."

## Context

The main agent started this worktree through the adapter for the active runtime, commonly `spawn-session.sh` for Claude Code or `start-codex-worktree.ps1` for Windows Codex.
